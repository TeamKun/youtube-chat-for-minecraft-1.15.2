...
class ChatService implements YouTubeChatService {
    ...
    public void stop(ICommandSource sender) {
        stopPolling();
        if (executor != null) {
            executor.shutdown();
            executor = null;
                ...
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            onComplete.accept(null);
                            showMessage(t.getMessage(), Minecraft.getInstance().player);
                            t.printStackTrace();
                        }
                    }
                });
                ...
                new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            Minecraft.getInstance().deferTask(onComplete);
                        } catch (Throwable t) {
                            showMessage(t.getMessage(), Minecraft.getInstance().player);
                            t.printStackTrace();
                            onComplete.run();
    ...
    private void showMessage(String message, ICommandSource sender) {
